...
const dbPath = path.join(process.cwd(), 'zentopup.db');
...
export function initDatabase() {
  ...
  try {
    sqlite.exec(`ALTER TABLE notifications ADD COLUMN link_href TEXT;`);
  } catch {}
  try {
    sqlite.exec(`ALTER TABLE notifications ADD COLUMN link_text TEXT;`);
  } catch {}
  try {
    sqlite.exec(`ALTER TABLE users ADD COLUMN avatar_url TEXT;`);
  } catch {}
  try {
    sqlite.exec(`ALTER TABLE users ADD COLUMN member_level TEXT DEFAULT 'Bronze';`);
  } catch {}
  try {
    sqlite.exec(`ALTER TABLE users ADD COLUMN role TEXT DEFAULT 'user';`);
  } catch {}
  try {
    sqlite.exec(`ALTER TABLE users ADD COLUMN updated_at TEXT DEFAULT CURRENT_TIMESTAMP;`);
  } catch {}
}
